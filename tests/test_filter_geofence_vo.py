...
from safety_sim.filters.geofence_vo import GeofenceVoFilter
...
def test_registered_in_filter_registry():
    assert "geofence_vo" in available_filters()
    assert make_filter("geofence_vo").name == "geofence_vo"
...
def test_reverse_toward_rear_wall_is_blocked():
    filt = make_filter_under_test()
    desired = Twist(-0.15, 0.0)
    decision = filt.filter(desired, obs(Pose(0.16, 0.0, 0.0),
                                        wheel_feedback=feedback_for(desired)),
                           t=0.0, dt=0.05)
    assert decision.mode in ("MODIFIED", "STOP")
    assert decision.cmd.v > desired.v
