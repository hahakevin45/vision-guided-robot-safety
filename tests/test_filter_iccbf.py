...
from safety_sim.filters.iccbf import IccbfFilter
...
def test_open_space_straight_command_passes_unchanged():
    ...
    decision = filt.filter(
        desired,
        obs(Pose(1.0, 0.0, 0.0), wheel_feedback=feedback_for(desired)),
        t=0.0,
        dt=0.05,
    )
...
def test_near_wall_full_speed_command_is_blocked():
    ...
    decision = filt.filter(
        desired,
        obs(Pose(3.84, 0.0, 0.0), wheel_feedback=feedback_for(desired)),
        t=0.0,
        dt=0.05,
    )
...
def test_reverse_toward_rear_wall_is_blocked():
    ...
    decision = filt.filter(
        desired,
        obs(Pose(0.16, 0.0, 0.0), wheel_feedback=feedback_for(desired)),
        t=0.0,
        dt=0.05,
    )
    assert decision.mode in ("MODIFIED", "STOP")
...
def test_missing_pose_and_stale_link_force_stop():
    ...
    no_pose = filt.filter(
        Twist(0.15, 0.0),
        obs(pose=None, pose_age=math.inf),
        t=0.0,
        dt=0.05,
    )
    stale_link = filt.filter(
        Twist(0.15, 0.0),
        obs(link_age=0.41),
        t=0.05,
        dt=0.05,
